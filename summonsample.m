...
for i=1:length(mu)
    ...
    elseif strcmp(dist{i},'Uniform')==1
        dummy1(i)=mu(i);
        dummy2(i)=sigma(i);
    end
end
...
for i = 1:n
    for j=1:nRV
        dummy0=cdf('Normal',u(i,j),0,1); %Prob
        if dummy0==1
            dummy0=1-1e-10;
        end

        if strcmp(dist{j},'Beta')==1
            sample(i,j)=dummy1(j)+(dummy2(j)-dummy1(j))*icdf(dist{j},dummy0,8,8);
        else
            sample(i,j)=icdf(dist{j},dummy0,dummy1(j),dummy2(j));
        end
