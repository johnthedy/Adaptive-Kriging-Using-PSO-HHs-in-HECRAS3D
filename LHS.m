function [xdoe,G_xdoe,nKs1]=LHS(nRV,nLHSs,nKs1,uxdoe,lxdoe,probtype,mu,sigma,dist,UOriginp,Unewinp,GOriginp,Gnewinp,PRJfile,HDFfile)
dummy3=ones(1,nRV).*0.5;
xdoe=lxdoe+dummy3.*(uxdoe-lxdoe);
[sample,~,~,~]=summonsample(1,mu,sigma,nRV,dist,xdoe);
G_xdoe=G(sample,probtype,UOriginp,Unewinp,GOriginp,Gnewinp,PRJfile,HDFfile);
...
while stopcrit<nLHSs
    ...
    for i=1:nKs1-dummy7
        newG_xdoe(i,:)=G(sample(i,:),probtype,UOriginp,Unewinp,GOriginp,Gnewinp,PRJfile,HDFfile);
    end
